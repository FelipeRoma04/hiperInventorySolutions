...
import com.hiper.inventory.dao.UbicacionDAO;
...
@WebServlet(name = "UbicacionServlet", urlPatterns = {"/api/ubicaciones/*"})
public class UbicacionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        if (nombre.isEmpty()) { json(res, Map.of("success", false, "message", "Nombre requerido"), 400); return; }
        if (dao.exists(nombre, 0)) { json(res, Map.of("success", false, "message", "Ya existe una ubicación con ese nombre"), 409); return; }
        int id = dao.create(nombre, param(req, "sede"), param(req, "descripcion"));
        if (id > 0) {
            logAction(req, "CREATE", "ubicaciones", id, nombre);
            json(res, Map.of("success", true, "id", id), 201);
        } else {
            json(res, Map.of("success", false, "message", "Error creando ubicación"), 500);
        }
    }
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        String nombre = param(req, "nombre");
        if (nombre.isEmpty()) { json(res, Map.of("success", false, "message", "Nombre requerido"), 400); return; }
        if (dao.exists(nombre, id)) { json(res, Map.of("success", false, "message", "Ya existe una ubicación con ese nombre"), 409); return; }
        boolean ok = dao.update(id, nombre, param(req, "sede"), param(req, "descripcion"));
        if (ok) { logAction(req, "UPDATE", "ubicaciones", id, nombre); json(res, Map.of("success", true), 200); }
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        if (ok) { logAction(req, "DELETE", "ubicaciones", id, null); json(res, Map.of("success", true), 200); }
